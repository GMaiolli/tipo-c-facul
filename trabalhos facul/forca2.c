...
int main() {
    ...
    char sublinhados[TAM_MAX_PALAVRA * 2 + 1]; 

    strcpy(pal, palavras[rand() % MAX_PALAVRAS]);
    for (size_t i = 0; i < strlen(pal) * 2; i += 2) { 
        sublinhados[i] = '_';
        sublinhados[i + 1] = ' ';
    }
    ...
    while (1) {
        if (tentativas >= max_tentativas) {
            printf("\nVoce excedeu o número máximo de tentativas. A palavra era: %s\n", pal);
            break;
        }
        ...
        if (strchr(letras_corretas, letra) != NULL || strchr(letras_erradas, letra) != NULL) {
            printf("Voce ja escolheu esta letra. Tente outra.\n");
            continue;
        }
